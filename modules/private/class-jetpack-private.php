...
class Jetpack_Private {
	...
	public static function privatize_blog( $wp ) {
		...
		if ( is_user_logged_in() && ( is_super_admin() || self::is_private_blog_user( $wpdb->blogid, $this_current_user ) ) ) {
			return;
		}
	 ...
	 */
	public static function is_private_blog_user( $blog, $user ) {
		global $wpdb;
	...
	public static function privatize_blog_priv_selector() {
		...
		<div class="jetpack-private__setting-disabled highlight">
			...
			wp_kses( sprintf( __( 'This setting is ignored because you <a href="%s">made your site private</a>', 'jetpack' ), admin_url( 'admin.php?page=jetpack' ) . '#/security?term=private' ), array( 'a' => array( 'href' => true ) ) );
			?>
		</div>
		<fieldset>
		<?php
	...
	public static function private_blog_prevent_requests() {
		...
		if ( ! self::is_private_blog_user( $wpdb->blogid, $this_current_user ) ) {
			wp_die( '', 403 );
		}
	}
